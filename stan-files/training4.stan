...
data {
    ...
    vector<lower=0,upper=1>[N] prev ; // previous response category
...
//     aResp ~ normal(bResp,sigma_Resp) ;
    {
        vector[N] yHat ;
        for(i in 1:N){
//             yHat[i]  <- a[S[i]] + b0[group[i]]*d0[i] + b2[group[i]]*d2[i] + bResp*prev[i] ;
           yHat[i]  <- a[S[i]] + a0[S[i]]*d0[i] + a2[S[i]]*d2[i] + bResp*prev[i] ;
// 	   yHat[i]  <- a[S[i]] + a0[S[i]]*d0[i] + a2[S[i]]*d2[i] + aD[S[i]]*iDiff[i] + aBlock[group[i]]*trialBlock[i] + aResp[S[i]]*prev[i]
        }
