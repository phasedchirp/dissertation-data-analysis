...
//         a[i] ~ student_t(n_a[gID[i]],alpha[gID[i]],sigma_a[gID[i]]) ;
    ...
    // d0 effects:
    b0 ~ normal(0,50) ;
//     n0 ~ exponential(0.033) ;
    for(i in 1:Nsubj){
        a0[i] ~ normal(b0[gID[i]],sigma_b0[gID[i]]) ;
//         a0[i] ~ student_t(n0,b0[gID[i]],sigma_b0[gID[i]]) ;
    }
    // d2 effects:
    b2 ~ normal(0,50) ;
//     n2 ~ exponential(0.03) ;
...
//         a2[i] ~ student_t(n2,b2[gID[i]],sigma_b2[gID[i]]) ;
    }
    bD ~ normal(0,50) ;
    for(i in 1:Nsubj){
...
//     real p_aBlock[G] ;
//     for (i in 1:G){
// 	p_alpha[i] <- student_t_rng(n_a,alpha[gID[i]],sigma_a)
//     }
//     for (i in 1:G){
// 	p_b0[i] <- student_t_rng(n0,b0[gID[i]],sigma_b0)
//     }
//     for (i in 1:G){
// 	p_b2[i] <- student_t_rng(n2,b2[gID[i]],sigma_b2)
//     }
//     p_bD <- normal_rng(
}
