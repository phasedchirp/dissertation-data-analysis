sample_files = list.files('.',full.names=TRUE,pattern='train-rslopes*')
...
qPred2 = function(block="1",resp="1"){
  pred2 = NULL
  sampleVec = sample(1:length(trainCoda[,1]),nSamps)
  for(i in sampleVec){
...
qPredB = function(block="1",resp="1"){
  ...
  for(i in sampleVec){
    ...
    if(block=="1"){
      ...
      predB = rbind(predB,(-(pars[2]+pars[14])/(pars[5]+comb*pars[11]) - comb*pars[8]/(pars[5]+comb*pars[11])))
    }
  }
  out = data.frame(d2 = comb)
...
qPred0 = function(block="1",resp="1"){
  ...
  for(i in sampleVec){
    ...
    if(block=="1"){
      ...
      pred0 = rbind(pred0,(-(pars[3]+pars[15])/(pars[6]+comb*pars[12]) - comb*pars[9]/(pars[6]+comb*pars[12])))
    }
  }
  out = data.frame(d2 = comb)
...
plot = ggplot(data=plotData,aes(x=d2,y=d0,color=group)) +
    ...
    facet_grid(~block,labeller = label_both)+theme(strip.text=element_text(size=10))
plot
